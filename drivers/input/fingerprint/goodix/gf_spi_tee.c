...
//add by zhangchunfa 
struct wake_lock gf_lock;	/* For suspend usage */
int is_screen_off = 0; 
/* -------------------------------------------------------------------- */
/* timer function								*/
/* -------------------------------------------------------------------- */
...
static int gf_get_gpio_dts_info(struct gf_device *gf_dev)
{
	...
	//gf_debug(ERR_LOG, "%s get device node reset_high !\n", __func__);
...
//#ifdef CONFIG_MTK_CLKMGR
	...
	if (bonoff && (count == 0)) {
		...
		secspi_enable_clk(gf_dev->spi);
		count = 1;
	} else if ((count > 0) && (bonoff == 0)) {
		//mt_spi_disable_clk(ms);
		secspi_disable_clk(gf_dev->spi);
		count = 0;
	}
//#endif
}
...
static void gf_hw_reset(struct gf_device *gf_dev, u8 delay)
{
	...
	//gf_debug(ERR_LOG, "%s, zhengwei pins_reset_low\n", __func__);
	mdelay(5);
	pinctrl_select_state(gf_dev->pinctrl_gpios, gf_dev->pins_reset_high);
	//gf_debug(ERR_LOG, "%s, zhengwei pins_reset_high\n", __func__);
#endif
...
static irqreturn_t gf_irq(int irq, void *handle)
{
    ...
    if(is_screen_off){
	    gf_debug(INFO_LOG, "%s: gf_lock will block  system in 500ms", __func__);
	    wake_lock_timeout(&gf_lock, HZ / 2);
    }
...
static int gf_probe(struct spi_device *spi)
{
    ...
    //add by zhangchunfa
	wake_lock_init(&gf_lock, WAKE_LOCK_SUSPEND, "gf wakelock");
