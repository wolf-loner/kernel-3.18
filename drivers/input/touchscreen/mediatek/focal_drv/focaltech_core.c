...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    if (retval != 0)
        FTS_ERROR("Failed to enable reg-vgp6: %d\n", retval);
#endif
    /* configure the gpio pins */
    retval = gpio_request_one(focal_data->tpd_rst_gpio_number, GPIOF_OUT_INIT_HIGH,
                 "touchp_reset");
    if (retval < 0) {
...
// gaoyuan add for td4100
    ...
    retval = fts_i2c_read_reg(fts_i2c_client, FTS_REG_CHIP_ID,&uc_reg_value);
...
#endif
    //fts_reset_proc();
    ;//retval = fts_i2c_read_reg(fts_i2c_client, 0xA6, &val);
    FTS_DEBUG("read 0xA6: %02X, ret: %d", val, retval);
