...
//step counter
#define BMI160_STEP_COUNTER

#ifdef BMI160_STEP_COUNTER
...
#define BMI160_CALI_TOLERANCE		(2500) //mg
...
/*----------------------------------------------------------------------------*/
static struct i2c_client *bmi160_acc_i2c_client = NULL;
...
#ifdef BMI160_STEP_COUNTER
static int bmi160_step_c_local_init(void);
static int bmi160_step_c_local_uninit(void);

static struct step_c_init_info  bmi160_step_c_init_info = {
	.name   = "bmi160_step_counter",
	.init   = bmi160_step_c_local_init,
	.uninit = bmi160_step_c_local_uninit,
	//.platform_diver_addr = &lsm6ds3_pedometer_driver,
};
static struct wake_lock bmi160_stepc_wake_lock; //send data to framework before sleep
#endif
...
static int BMI160_ACC_ReadData(struct i2c_client *client, s16 data[BMI160_ACC_AXES_NUM])
{
	...
	err = /*bma_i2c_read_block*/hwmsen_read_block(client, addr, buf, BMI160_ACC_DATA_LEN);
	if(err) {
		GSE_ERR("error: %d\n", err);
	}
	else
...
#else
	err = /*bma_i2c_read_block*/hwmsen_read_block(client, BMI160_ACC_REG_OFSX, ofs, BMI160_ACC_AXES_NUM);
	if(err) {
		GSE_ERR("error: %d\n", err);
	}
#endif
...
static int BMI160_ACC_ResetCalibration(struct i2c_client *client)
{
	...
	#else
	u8 ofs[4]={0,0,0,0};
	err = /*bma_i2c_write_block*/hwmsen_write_block(client, BMI160_ACC_REG_OFSX, ofs, 4);
	if(err) {
		GSE_ERR("error: %d\n", err);
	}
	#endif
...
static int BMI160_ACC_WriteCalibration(struct i2c_client *client, int dat[BMI160_ACC_AXES_NUM])
{
	...
	err = /*bma_i2c_write_block*/hwmsen_write_block(obj->client, BMI160_ACC_REG_OFSX, obj->offset, BMI160_ACC_AXES_NUM);
	if(err) {
...
static int BMI160_ACC_CheckDeviceID(struct i2c_client *client)
{
...
#if 0
	res = /*bma_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_CHIP_ID__REG, databuf, 0x01);
#endif
	res = /*bma_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_CHIP_ID__REG, databuf, 0x01);
	//res = hwmsen_read_byte(client,BMI160_USER_CHIP_ID__REG,databuf);
	if(res < 0)
	...
	case SENSOR_CHIP_ID_BMI_C3:
		GSE_LOG("BMI160_ACC_CheckDeviceID 0x%x pass!\n ", databuf[0]);
		break;
	default:
		GSE_LOG("BMI160_ACC_CheckDeviceID 0x%x failt!\n ", databuf[0]);
		break;
	}
...
static int BMI160_ACC_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	{
		GSE_LOG("Gsensor power status is newest!\n");
		return BMI160_ACC_SUCCESS;
	}
	...
	{
	    if (atomic_read(&obj->step_c_enabled) == 0)
	    {
	...
	res = /*bma_i2c_write_block*/hwmsen_write_block(client,
	...
	GSE_LOG("bmi160-acc enable(%d), ACC_PMU_STATUS_CMD=%#x", enable, databuf[0]);
	sensor_power = enable;
	mdelay(50);
	//mutex_unlock(&obj->lock);
...
static int BMI160_ACC_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	mutex_lock(&obj->lock);
	res = /*bma_i2c_read_block*/hwmsen_read_block(client,
...
static int BMI160_ACC_SetIntEnable(struct i2c_client *client, u8 intenable)
{
	...
	mutex_lock(&obj->lock);
	res = /*bma_i2c_write_block*/hwmsen_write_block(client, BMI160_USER_INT_EN_0_ADDR, &intenable, 0x01);
	mdelay(1);
	...
	res = /*bma_i2c_write_block*/hwmsen_write_block(client, BMI160_USER_INT_EN_1_ADDR, &intenable, 0x01);
	mdelay(1);
	if(res != BMI160_ACC_SUCCESS)
	{
		mutex_unlock(&obj->lock);
	...
	res = /*bma_i2c_write_block*/hwmsen_write_block(client, BMI160_USER_INT_EN_2_ADDR, &intenable, 0x01);
	mdelay(1);
	if(res != BMI160_ACC_SUCCESS)
	{
		mutex_unlock(&obj->lock);
...
/*----------------------------------------------------------------------------*/
/*---add by dgsh for reset step counter when reset phone.20160825--------start-----------------------*/
static int BMI160_ACC_ResetStepCounter(struct i2c_client *client)
{
    int res = 0;
	unsigned char databuf[2] = {0};
	struct bmi160_acc_i2c_data *obj = obj_i2c_data;
    databuf[0] = BMI160_CMD_COMMANDS__CLEAR_STEP;
	mutex_lock(&obj->lock);
	res = /*bma_i2c_write_block*/hwmsen_write_block(client, BMI160_CMD_COMMANDS__REG, &databuf[0], 1);
	mdelay(5);
	if(res != BMI160_ACC_SUCCESS)
	{
		mutex_unlock(&obj->lock);
...
static int bmi160_acc_init_client(struct i2c_client *client, int reset_cali)
{
	...
	res = BMI160_ACC_SetDataFormat(client, BMI160_ACCEL_RANGE_4G);
	if(res != BMI160_ACC_SUCCESS)
	{
		return res;
	}
    ...
    res = BMI160_ACC_ResetStepCounter(client);
	if(res != BMI160_ACC_SUCCESS)
	{
		return res;
	}
	GSE_LOG("BMI160_ACC reset stepcounter data!\n");
...
static int BMI160_ACC_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
		...
		databuf[BMI160_ACC_AXIS_X] = databuf[BMI160_ACC_AXIS_X] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
		databuf[BMI160_ACC_AXIS_Y] = databuf[BMI160_ACC_AXIS_Y] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
		databuf[BMI160_ACC_AXIS_Z] = databuf[BMI160_ACC_AXIS_Z] * GRAVITY_EARTH_1000 / obj->reso->sensitivity;
...
static int bmi160_acc_set_mode(struct i2c_client *client, unsigned char mode)
{
	...
	case BMI160_ACC_MODE_NORMAL:
		databuf[0] = CMD_PMU_ACC_NORMAL;
		comres += /*bma_i2c_write_block*/hwmsen_write_block(client,
				BMI160_CMD_COMMANDS__REG, &databuf[0], 1);
		break;
...
static int bmi160_acc_get_mode(struct i2c_client *client, unsigned char *mode)
{
	...
	comres = /*bma_i2c_read_block*/hwmsen_read_block(client,
...
static int bmi160_acc_get_range(struct i2c_client *client, unsigned char *range)
{
	...
	comres = /*bma_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_ACC_RANGE__REG,	&data, 1);
...
static int bmi160_acc_get_bandwidth(struct i2c_client *client, unsigned char *bandwidth)
{
	...
	comres = /*bma_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_ACC_CONF_ODR__REG, bandwidth, 1);
...
static ssize_t show_power_status_value(struct device_driver *ddri, char *buf)
{
    ...
    len += snprintf(buf+len, PAGE_SIZE-len, "sensor_power = %d\n", sensor_power);
	return len;
}
...
static int bmi160_fifo_length(uint32_t *fifo_length)
{
	...
	comres += /*bma_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_FIFO_BYTE_COUNTER_LSB__REG, a_data_u8r, 2);
...
int bmi160_set_command_register(u8 cmd_reg)
{
	...
	comres += /*bma_i2c_write_block*/hwmsen_write_block(client, BMI160_CMD_COMMANDS__REG, &cmd_reg, 1);
...
static int bmi160_fifo_data_sel_get(struct bmi160_acc_i2c_data *client_data)
{
	...
	err = /*bma_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_FIFO_ACC_EN__REG, &data, 1);
	fifo_acc_en = BMI160_GET_BITSLICE(data, BMI160_USER_FIFO_ACC_EN);

	err += /*bma_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_FIFO_GYRO_EN__REG, &data, 1);
	fifo_gyro_en = BMI160_GET_BITSLICE(data, BMI160_USER_FIFO_GYRO_EN);

	err += /*bma_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_FIFO_MAG_EN__REG, &data, 1);
...
static ssize_t bmi160_fifo_data_sel_store(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	err += /*bma_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_FIFO_ACC_EN__REG, &fifo_datasel, 1);
	fifo_datasel = BMI160_SET_BITSLICE(fifo_datasel, BMI160_USER_FIFO_ACC_EN, fifo_acc_en);
	err += /*bma_i2c_write_block*/hwmsen_write_block(client, BMI160_USER_FIFO_ACC_EN__REG, &fifo_datasel, 1);

	err += /*bma_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_FIFO_GYRO_EN__REG, &fifo_datasel, 1);
	fifo_datasel = BMI160_SET_BITSLICE(fifo_datasel, BMI160_USER_FIFO_GYRO_EN, fifo_gyro_en);
	err += /*bma_i2c_write_block*/hwmsen_write_block(client, BMI160_USER_FIFO_GYRO_EN__REG, &fifo_datasel, 1);

	err += /*bma_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_FIFO_MAG_EN__REG, &fifo_datasel, 1);
	fifo_datasel = BMI160_SET_BITSLICE(fifo_datasel, BMI160_USER_FIFO_MAG_EN, fifo_mag_en);
	err += /*bma_i2c_write_block*/hwmsen_write_block(client, BMI160_USER_FIFO_MAG_EN__REG, &fifo_datasel, 1);
...
#else
		err = /*bma_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_FIFO_DATA__REG, 
				g_fifo_data_arr, client_data->fifo_bytecount);
#endif
	} else
...
static ssize_t show_full_regs(struct device_driver *ddri, char *buf)
{
	...
	/*
    bma_i2c_read_block(client, 0x03, buffer, 0x01); 	
	len += snprintf(buf + len, PAGE_SIZE - len, "[0x03]:0x%02x\n", buffer[0]);	
		
	for (i = 0x40; i <= 0x43; i++)
	{
		bma_i2c_read_block(client, i, buffer, 0x01); 	
		len += snprintf(buf + len, PAGE_SIZE - len, "[0x%02x]:0x%02x\n", i, buffer[0]);		
	}
	*/
	for (i = 0x00; i <= 0x7B; i++)
	{
		/*bma_i2c_read_block*/hwmsen_read_block(client, i, buffer, 0x01); 	
		len += snprintf(buf + len, PAGE_SIZE - len, "[0x%02x]:0x%02x\n", i, buffer[0]);		
	}
	return len;    
...
/*----------------------------------------------------------------------------*/
static ssize_t store_spec_reg(struct device_driver *ddri, const char *buf, size_t count)
{
	struct i2c_client *client = obj_i2c_data->client;
	u8 databuf[2]={0};
	unsigned int addr = 0, data_in = 0;
	int res = 0;
	GSE_FUN();
	printk("%s: buf = %s\n", __FUNCTION__, buf);
	if (2 == sscanf(buf, "0x%x 0x%x", &addr, &data_in))
	{
		databuf[0] = addr;
		databuf[1] = data_in;
		GSE_LOG("%s: databuf[0]=0x%2x  databuf[1]=0x%2x \n", __FUNCTION__, databuf[0],databuf[1]);

		res = i2c_master_send(client, databuf, 0x2);

		if(res <= 0)
		{			
			GSE_LOG("%s: change reg fail - i2c err\n", __FUNCTION__);
		}
		else
		{
			GSE_LOG("%s: change reg ok\n", __FUNCTION__);
		}
	}
	else
	{
		GSE_LOG("%s: chage reg fail - arguement not match\n",__FUNCTION__);
	}
	return (res < 0)? BMI160_ACC_ERR_I2C : count;
...
static DRIVER_ATTR(regs, S_IRUGO | S_IWUSR, show_full_regs, store_spec_reg);
/*----------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------*/
//acc calibrate proc, add by zte szy 20161109  --start--
static bool is_accel_calibration_valid(int cali_value[BMI160_ACC_AXES_NUM])
{
	// cali_value's unit is mg.
	if ((abs(cali_value[BMI160_ACC_AXIS_X]) > BMI160_CALI_TOLERANCE) 
		|| (abs(cali_value[BMI160_ACC_AXIS_Y]) > BMI160_CALI_TOLERANCE)
		|| (abs(cali_value[BMI160_ACC_AXIS_Z]) > BMI160_CALI_TOLERANCE))
	{
		return false;
	}
	return true;
...
static ssize_t acc_calibrate_read_proc(struct file *file,	char __user *buffer, size_t count, loff_t *offset)
{
	...
	// calc avarage
	GSE_LOG("sum,x:%d,y:%d,z:%d\n", raw_sum[BMI160_ACC_AXIS_X], raw_sum[BMI160_ACC_AXIS_Y], raw_sum[BMI160_ACC_AXIS_Z]);
	...
	GSE_LOG("avg,x:%d,y:%d,z:%d\n",rawacc[BMI160_ACC_AXIS_X],rawacc[BMI160_ACC_AXIS_Y],rawacc[BMI160_ACC_AXIS_Z]);
	...
	cali_data[obj->cvt.map[BMI160_ACC_AXIS_Z]] = GRAVITY_EARTH_1000 - obj->cvt.sign[BMI160_ACC_AXIS_Z] * rawacc[BMI160_ACC_AXIS_Z];
	...
	BMI160_ACC_ResetCalibration(bmi160_acc_i2c_client);
	BMI160_ACC_WriteCalibration(bmi160_acc_i2c_client, cali_data);
	
	cnt = sprintf(buffer, "%d %d %d", cali_data[BMI160_ACC_AXIS_X], cali_data[BMI160_ACC_AXIS_Y], cali_data[BMI160_ACC_AXIS_Z]);
...
static ssize_t acc_calibrate_write_proc(struct file *file, const char __user *user_buf, size_t len, loff_t *offset)
{
	...
	sscanf(buf, "%d %d %d", &cali_data[BMI160_ACC_AXIS_X], &cali_data[BMI160_ACC_AXIS_Y], &cali_data[BMI160_ACC_AXIS_Z]);
...
//acc calibrate proc, add by zte szy 20161109  --e-n-d--
	...
	switch(cmd)
	{
		...
		case GSENSOR_IOCTL_SET_CALI:
			...
			else
			{
			    ...
			    GSE_LOG("%s:[%d,%d,%d]",__func__,cali[0],cali[1],cali[2]);
			#endif
			}
			break;
...
static int bmi160_acc_resume(struct i2c_client *client)
{
	...
	err = BMI160_ACC_SetPowerMode(obj->client, power_status);
	if(err)
	{
		GSE_ERR("resume client fail! err code %d!\n", err);
		return err ;        
	}
/*	
...
#ifdef BMI160_STEP_COUNTER
static int bmi160_enable_pedo(struct i2c_client *client, bool enable)
{
	u8 databuf[2] = {0};    
	int res = 0;
	...
	if(true == enable)
	{	
	    databuf[0] = BMI160_PEDO_THRESH_ADDR;
		databuf[1] = BMI160_PEDO_THRESH_TIME;
		...
		if (false == power_status)
		{
		    GSE_LOG("enable acc for step counter\n");
		    res = bmi160_acc_enable_nodata(1);
	        if(res) 
			{
			    GSE_ERR("initialize client fail!!\n");
				return res;
		...
		databuf[0] = BMI160_PEDO_EN_BUF_ADDR;
		databuf[1] = (BMI160_PEDO_STEP_BUF) | (BMI160_PEDO_ENABLE);
		res = i2c_master_send(client, databuf, 0x02);
		if(res < 0)
	    {
		    GSE_ERR("write enable pedometer func register err!\n");
	...
	/*	
		res = bmi160_reset_pedo_data(client);
		
		if(res != BMI160_ACC_SUCCESS)
		{
			GSE_LOG(" BMI160_Reset_Pedo_Data failed!\n");
			return -1;
		}
	*/
	}
	...
	{	 
	    databuf[0] = BMI160_PEDO_EN_BUF_ADDR;
		databuf[1] = BMI160_PEDO_STEP_BUF;
		res = i2c_master_send(client, databuf, 0x02);
		if(res < 0)
	    {
		    GSE_ERR("write enable pedometer func register err!\n");
...
static int bmi160_clear_pedo(struct i2c_client *client)
{
    int res = 0;
	u8 databuf[2];
	databuf[0] = BMI160_CMD_COMMANDS__REG;
	databuf[1] = BMI160_CMD_COMMANDS__CLEAR_STEP;
	res = i2c_master_send(client, databuf, 0x02);
	if(res < 0)
	{
		GSE_ERR("write enable pedometer func register err!\n");
...
static int bmi160_step_c_enable_nodata(int en)
{
    ...
    res = atomic_read(&priv->step_c_enabled);
	if (res == value)
	{
        GSE_LOG("step_counter is already new status.\n");
		return 0;
	}

	if(value == 1)
	{
		res = bmi160_enable_pedo(priv->client, true);
		if(BMI160_ACC_SUCCESS != res)
		{
			GSE_LOG("bmi160_enable_pedo failed at open action!\n");
			return res;
		}
		atomic_set(&priv->step_c_enabled, 1);
	}
	else
	{
		res = bmi160_enable_pedo(priv->client, false);
		if(BMI160_ACC_SUCCESS != res)
		{
		...
		atomic_set(&priv->step_c_enabled, 0);
	}
	
	GSE_LOG("bmi160_step_c_enable_nodata %d OK!\n", value);
    return err;
}
...
static int bmi160_step_d_set_delay(u64 delay)
{
	//Null
	return 0;
}

static int bmi160_step_c_set_delay(u64 delay)
{
	//Null
	return 0;
}
...
static int bmi160_step_c_get_data(uint32_t *value, int *status)
{
	...
	res = /*bma_i2c_read_block*/hwmsen_read_block(priv->client, BMI160_PEDO_DATA_ADDR, databuf, 0x02);
	if (res)
	{
	    GSE_ERR("LSM6DS3 read stepC data  error\n");
		return -1;
	}
	regdata = (databuf[1]<< 8) | databuf[0];
	//GSE_LOG("bmi160-step : %d - %#x, %#x, %#x\n", regdata, regdata, databuf[0], databuf[1]);
	wake_lock(&bmi160_stepc_wake_lock); 
	
	*value = regdata;// + priv->current_step;
	*status = SENSOR_STATUS_ACCURACY_MEDIUM;
	wake_unlock(&bmi160_stepc_wake_lock); 
	//send data to framework before sleep end

	return res;
}
static int bmi160_step_c_get_data_step_d(uint32_t *value, int *status)
{
	return 0;
}
static int bmi160_step_c_get_data_significant(uint32_t *value, int *status)
{
	return 0;
}
#endif
...
#ifdef MISC_FOR_DAEMON
	int err = 0;
	GSE_LOG("en = %d, power_status(bf) = %d, sensor_power = %d\n",
			en, power_status, sensor_power);
...
*/
	...
	else
	{
		...
		{
		    res = BMI160_ACC_SetPowerMode(obj_i2c_data->client, power_status);
		    if(res == 0)
		    {
			    GSE_LOG("BMI160_ACC_SetPowerMode done[%d]\n", retry);
			    break;
		    }
		    GSE_LOG("BMI160_ACC_SetPowerMode fail[%d].\n", retry);
	    }
	}
...
static int bmi160_acc_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	atomic_set(&obj->suspend, 0);
#ifdef BMI160_STEP_COUNTER	
	atomic_set(&obj->step_c_enabled, 0);
	wake_lock_init(&bmi160_stepc_wake_lock, WAKE_LOCK_SUSPEND, "lsm6ds3_stepc_wake_locks");
#endif	
	mutex_init(&obj->lock);
	//tad3sgh add ++
    ...
    bmi160_acc_i2c_client->addr = hw->i2c_addr[0];
...
static int bmi160_step_c_local_init(void)
{
	...
	{
		GSE_LOG("%s,bmi160_acc_init_flag == -1.\n", __FUNCTION__);
		err = -1;
	}
	else
	{
		step_ctl.open_report_data= bmi160_step_c_open_report_data;
		step_ctl.enable_nodata = bmi160_step_c_enable_nodata;
		step_ctl.enable_step_detect  = bmi160_step_c_enable_step_detect;
		step_ctl.step_c_set_delay = bmi160_step_c_set_delay;
		step_ctl.step_d_set_delay = bmi160_step_d_set_delay;
		step_ctl.enable_significant = bmi160_step_c_enable_significant;
		...
		step_data.get_data = bmi160_step_c_get_data;
		step_data.get_data_step_d = bmi160_step_c_get_data_step_d;
		step_data.get_data_significant = bmi160_step_c_get_data_significant;
		...
		bmi160_clear_pedo(bmi160_acc_i2c_client);
	}
...
#ifdef BMI160_STEP_COUNTER
	step_c_driver_add(&bmi160_step_c_init_info); //step counter
#endif
