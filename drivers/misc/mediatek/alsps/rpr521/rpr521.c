...
static DEFINE_MUTEX(rpr521_mutex);  //protect REG_MODECONTROL
...
static long rpr521_enable_als(struct i2c_client *client, int enable)
{
	...
	if(enable)
	{
	    //if als be turned off by suspend, not return directly for resume can turn on it.
		if((rpr521_als_enabled == CTL_STANDALONE)&&(atomic_read(&obj->als_suspend) == 0))
		{
...
/*
		...
		res = i2c_master_recv(client, databuf, 0x1);
*/
        res = i2c_smbus_read_byte_data(client, REG_SYSTEMCONTROL);
		if(res < 0)
		{
			return -1;
		}
...
/*		
		...
		res = i2c_master_recv(client, databuf, 0x1);
*/
        res = i2c_smbus_read_byte_data(client, REG_INTERRUPT);
		if(res < 0)
		{
			return -1;
		}
...
//	mutex_lock(&rpr521_mutex);
	...
	//	i2c_master_recv(client, &reg_val, 0x1);	
	    reg_val = i2c_smbus_read_byte_data(client, buffer[0]);
...
//	mutex_unlock(&rpr521_mutex);
    ret = i2c_smbus_write_byte_data(client, buffer[0], buffer[1]);
	if (ret < 0) {
...
//	APS_DBG("ROHM rpr521--ps_data=%d, low:%d  high:%d", *data, ps_value_low[0], ps_value_high[0]); 
	APS_DBG("ROHM rpr521--mode=%x, int_sts:%x", mode, int_sts);
...
*/	
    databuf = ps_tl & REG_PSTL_MAX;
	res = i2c_smbus_write_word_data(client, REG_PSTL_LSB, databuf);
	if(res < 0)
	{
		goto I2CERR;
	}
...
//    mutex_unlock(&rpr521_mutex);
*/
	databuf = ps_th & REG_PSTH_MAX;
	res = i2c_smbus_write_word_data(client, REG_PSTH_LSB, databuf);
	if(res < 0)
	{
		goto I2CERR;
	}
...
I2CERR:
//	mutex_unlock(&rpr521_mutex);
	APS_ERR("%s: rpr521 i2c err!", __func__);
	return rpr521_ERR_I2C;
}
...
static int rpr521_get_ps_value(struct rpr521_priv *obj, u16 ps)
{
	...
	APS_DBG("RPR521-PS:  %05d => %05d (invalid[%d])\n", ps, val, invalid);
...
*/	
	...
	//after "rpr521_enable_als" for resume can turn on als.
...
/*
     ...
     res = i2c_master_send(rpr521_i2c_client, databuf, 0x2);
*/
    databuf = ps_th_l & 0xFFF;
    res = i2c_smbus_write_word_data( rpr521_i2c_client, REG_PSTL_LSB, databuf);
     if(res < 0)
     {
         APS_ERR("%s: set lower threshold i2c err.\n", __func__);
         goto EXIT_ERR;
     }
/*
     databuf[0] = REG_PSTH_LSB;
     ...
     res = i2c_master_send(rpr521_i2c_client, databuf, 0x2);
*/
    databuf = ps_th_h & 0xFFF;
    res = i2c_smbus_write_word_data(rpr521_i2c_client, REG_PSTH_LSB, databuf);
     if(res < 0) 
     {
         APS_ERR("%s: set higher threshold i2c err.\n", __func__);
         goto EXIT_ERR;
     }
//    mutex_unlock(&rpr521_mutex);
	obj->ps_th_h = ps_th_h;
