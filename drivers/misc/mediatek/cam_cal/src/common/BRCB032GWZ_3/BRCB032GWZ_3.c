...
static int iReadCAM_CAL(u16 a_u2Addr, u8 * a_puBuff,u16 i2c_id)
{
    int  i4RetValue = 0;
    char puReadCmd[2] = {(char)(a_u2Addr >> 8) , (char)(a_u2Addr & 0xFF)};
	g_pstI2Cclient->addr = (BRCB032GWZ_3_DEVICE_ID>> 1);
//printk("===zlx=16bit==iReadCAM_CAL \n");
 ...
 void BRCB032GWZ_ReadOtp(kal_uint16 address,unsigned char *iBuffer,unsigned int buffersize)
{
		kal_uint16 i = 0;
		u8 readbuff;
		int ret ;
		//base_add=(address/10)*16+(address%10);
		//printk("==lhl=0525=BRCB032GWZ_ReadOtp 8bit\n");
		//printk("[CAM_CAL]ENTER page:0x%x address:0x%x buffersize:%d\n ",page, address, buffersize);
	//	if (OV13853_GotoPage(page))
		{
			for(i = 0; i<buffersize; i++)
			{				
				ret= iReadCAM_CAL(address+i,&readbuff,0xA0);
				//CAM_CALDB("[CAM_CAL]address+i = 0x%x,readbuff = 0x%x\n",(address+i),readbuff );
				//printk("==lhl=16bit= [0x%x]= 0x%x\n",(address+i),readbuff );
