...
static void glLoadNvram(IN P_GLUE_INFO_T prGlueInfo, OUT P_REG_INFO_T prRegInfo)
{
	...
	if (kalCfgDataRead16(prGlueInfo, sizeof(WIFI_CFG_PARAM_STRUCT) - sizeof(UINT_16), (PUINT_16) aucTmp) == TRUE) {
		...
		prRegInfo->ucSupport5GBand = 0;//aucTmp[1]; change by yangdeqing for CTS Wifi-direct fail
		...
		prRegInfo->ucEnable5GBand = 0;//aucTmp[0];change by yangdeqing for CTS Wifi-direct fail
		...
		prRegInfo->fg2G4BandEdgePwrUsed = 0;// (BOOLEAN) aucTmp[0];change by yangdeqing for CTS Wifi-direct fail
#if 0		//change by yangdeqing for CTS Wifi-direct fail
		if (aucTmp[0]) {
...
#endif  //change by yangdeqing for CTS Wifi-direct fail
