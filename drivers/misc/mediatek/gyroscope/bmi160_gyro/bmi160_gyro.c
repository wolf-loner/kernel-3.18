...
#define BMI_GYRO_TAG                  "[BMI160-GYRO] "

#define BMI_GYRO_FUN(f)               printk( BMI_GYRO_TAG"%s\n", __FUNCTION__)
#define BMI_GYRO_ERR(fmt, args...)    printk(KERN_ERR BMI_GYRO_TAG"%s %d : "fmt, __FUNCTION__, __LINE__, ##args)
#define BMI_GYRO_LOG(fmt, args...)    printk( BMI_GYRO_TAG fmt, ##args)
...
static struct i2c_client *bmi160_gyro_i2c_client = NULL;
static struct bmg_i2c_data *obj_i2c_data = NULL;
...
static int bmg_read_raw_data(struct i2c_client *client, s16 data[BMG_AXES_NUM])
{
	...
	if (priv->sensor_type == BMI160_GYRO_TYPE) {
		...
		err = /*bmg_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_DATA_8_GYR_X_LSB__REG,
				buf_tmp, 6);
		if (err) {
...
static int bmg_get_chip_type(struct i2c_client *client)
{
	...
	/* twice */
	err = /*bmg_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_CHIP_ID__REG, &chip_id, 0x01);
	err = /*bmg_i2c_read_block*/hwmsen_read_block(client, BMI160_USER_CHIP_ID__REG, &chip_id, 0x01);
	if (err != 0)
		return err;
	...
	case SENSOR_CHIP_ID_BMI_C3:
		...
		sprintf(obj->sensor_name, "bmi160_gyro");
		break;
	default:
		...
		enum BMG_POWERMODE_ENUM power_mode)
{
	...
	if (obj->sensor_type == BMI160_GYRO_TYPE) {
		...
		err += /*bmg_i2c_write_block*/hwmsen_write_block(client,
	...
	if (err < 0)
        ...
        BMI_GYRO_LOG("set power mode ok, sensor name = %s\n",obj->sensor_name);
	}
	mutex_unlock(&obj->lock);
	return err;
}
...
static int bmg_set_range(struct i2c_client *client, enum BMG_RANGE_ENUM range)
{
	...
	if (obj->sensor_type == BMI160_GYRO_TYPE) {/* BMI160_GYRO */
		...
		err = /*bmg_i2c_read_block*/hwmsen_read_block(client,
...
static int bmg_init_client(struct i2c_client *client, int reset_cali)
{
	...
	int err = 0;
	BMI_GYRO_FUN(f);
	BMI_GYRO_LOG("gyro client->addr = 0x%x\n", client->addr);
		...
		char *buf, int bufsize)
{
	...
	if (err) {
		BMI_GYRO_ERR("bmg read raw data failed, err = %d\n", err);
		return -3;
	} else {
		/* compensate data */
		...
		gyro[BMG_AXIS_X] = gyro[BMG_AXIS_X] * DEGREE_TO_RAD_SCALE / obj->sensitivity;
		gyro[BMG_AXIS_Y] = gyro[BMG_AXIS_Y] * DEGREE_TO_RAD_SCALE / obj->sensitivity;
		gyro[BMG_AXIS_Z] = gyro[BMG_AXIS_Z] * DEGREE_TO_RAD_SCALE / obj->sensitivity;
...
static ssize_t show_chipinfo_value(struct device_driver *ddri, char *buf)
{
	...
	if (NULL == obj) {
		BMI_GYRO_ERR("bmg i2c data pointer is null\n");
		return 0;
	}
		...
		const char *buf, size_t count)
{
	...
	if (!strncmp(buf, "rst", 3)) {
		...
		if (err) {
			BMI_GYRO_ERR("bmg write calibration failed, err = %d\n",
				err);
		}
	} else {
...
static int bmg_suspend(struct i2c_client *client, pm_message_t msg)
{
	...
	if (msg.event == PM_EVENT_SUSPEND) {
		...
		if (err) {
			BMI_GYRO_ERR("bmg set suspend mode failed, err = %d\n",
				err);
			return err;
		}
		bmg_power(obj->hw, 0);
...
*/
    err = bmg_set_powermode(obj_i2c_data->client, power_status);
	if (err) {
...
static int bmi160_gyro_enable_nodata(int en)
{
    ...
    if (power_status == obj_i2c_data->power_mode)
    {
        BMI_GYRO_LOG("GYRO device have updated!");
    }
	for(retry = 0; retry < 3; retry++){
		res = bmg_set_powermode(obj_i2c_data->client, power_status);
		if(res == 0)
		{
		...
		const struct i2c_device_id *id)
{
	...
	if (atomic_read(&obj->firlen) > 0)
		atomic_set(&obj->fir_en, 1);
#endif
	bmi160_gyro_i2c_client = obj->client;
	bmi160_gyro_i2c_client->addr = hw->i2c_addr[0];
...
static int bmg_i2c_remove(struct i2c_client *client)
{
	...
	if (err)
		BMI_GYRO_ERR("bmg_delete_attr failed, err = %d\n", err);
...
static int __init bmg_init(void)
{
	...
	if(!hw) {
		BMI_GYRO_ERR("Device tree configuration error!!\n");
		return 0;
	}
