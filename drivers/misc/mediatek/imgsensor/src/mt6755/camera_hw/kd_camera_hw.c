...
PowerUp PowerOnList = {
	...
	{SENSOR_DRVNAME_S5K4H8_MIPI_RAW,
	  ...
	  {SENSOR_DRVNAME_S5K4H8S_MIPI_RAW,
	  {
	   {SensorMCLK, Vol_High, 0},
	   {DOVDD, Vol_1800, 0},
	   {AVDD, Vol_2800, 0},
...
//		if (PowerCustList.PowerCustInfo[CUST_AVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
		if (pinSetIdx==1) {
		 ...
		 if (pinSetIdx == 1) {
			...
			if (1) {
				 PK_ERR("[CAMERA SENSOR] Main camera VCAM_D power on\n"); 
				if (TRUE != _hwPowerOn(pwInfo.PowerType, pwInfo.Voltage)) {
					PK_ERR("[CAMERA SENSOR] Fail to enable digital power\n");
					return FALSE;
				}
		...
		PK_ERR("[CAMERA SENSOR] camera DVDD power on\n"); 
		...
		if (PowerCustList.PowerCustInfo[CUST_AFVDD].Gpio_Pin == GPIO_UNSUPPORTED) {
			PK_ERR("hwpoweron--lhl-afvdd---1221----13M-\n");
			if (TRUE != _hwPowerOn(pwInfo.PowerType, pwInfo.Voltage)) {
...
{
	if (pwInfo.PowerType == AVDD) {
		if ((PowerCustList.PowerCustInfo[CUST_AVDD].Gpio_Pin == GPIO_SUPPORTED)&&(pinSetIdx==1)) {	
			if (TRUE != _hwPowerDown(pwInfo.PowerType)) {
				PK_ERR("[CAMERA SENSOR] Fail to disable analog power\n");
				return FALSE;
			}
		} else {
		PK_DBG("hwpowerdown--lhl-avdd---pinSetIdx=%d-3p3-222-\n",pinSetIdx);

			if (mtkcam_gpio_set(pinSetIdx, pwInfo.PowerType, 0)) {
