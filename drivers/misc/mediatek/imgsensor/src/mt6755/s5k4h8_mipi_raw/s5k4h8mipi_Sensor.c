...
/*************************************************************************
* FUNCTION
*    S5K4H8_OTP_combination_read
*
* DESCRIPTION
*    S5K4H8 otp combination( lens id is different)
*
**************************************************************************/

static UINT32 S5K4H8_OTP_combination_read(void)
{
	int lens_id_1=0;
	int lens_id_2=0;
	...
	if (0x1 == lens_id_1||0x1== lens_id_2)
	{
	printk(" chenyunchuan===s5k4h8 is old lens! \n");
	return TRUE;
	}
	else
	{
	printk(" chenyunchuan===s5k4h8 err! \n");
	return FALSE;
	}	
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12);
			...
			LOG_INF("ZLX-chenyunchuanget_s5k4h8_id(): %d\n ", *sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {               
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);      
               // return ERROR_NONE;
               lens_otp=S5K4H8_OTP_combination_read();	
		if(lens_otp)
		{
			 printk("ZLX-chenyunchuan===s5k4h8--OK--i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			printk("ZLX-chenyunchuan===s5k4h8-sensor and lens_id both matched!!!\n ");
			return ERROR_NONE;
		}
		else
		{
			*sensor_id = 0xFFFFFFFF;
			 printk("ZLX-chenyunchuan===-s5k4h8 sensor id matched, but lens_id is err! \n");
