...
#define Golden_RG   0x286 //0x206  //0x209  // modify by gionee wangxiaoyu 2015-09-01
#define Golden_BG   0x258 //0x24B  //0x259  // modify by gionee wangxiaoyu 2015-09-01
...
bool get_otp_wb(int zone)
{
	...
	#endif
	if(zone == 1){
		templ = s5k4h8_read_cmos_sensor(0x0A0C);// + (zone*WB_OFFSET));
		temph= s5k4h8_read_cmos_sensor(0x0A0D);// + (zone*WB_OFFSET));

		Current_RG = (USHORT)((temph<<8)| templ);

		templ = s5k4h8_read_cmos_sensor(0x0A0E);//+ (zone*WB_OFFSET));
		temph= s5k4h8_read_cmos_sensor(0x0A0F);//+ (zone*WB_OFFSET));

		Current_BG = (USHORT)((temph<<8)| templ);
	}
	if(zone == 2){
		templ = s5k4h8_read_cmos_sensor(0x0A22);// + (zone*WB_OFFSET));
		temph = s5k4h8_read_cmos_sensor(0x0A23);// + (zone*WB_OFFSET));
	
		Current_RG = (USHORT)((temph<<8)| templ);
	
		templ = s5k4h8_read_cmos_sensor(0x0A24);//+ (zone*WB_OFFSET));
		temph = s5k4h8_read_cmos_sensor(0x0A25);//+ (zone*WB_OFFSET));
