...
#define PFX "S5K4H8S_camera_sensor"
...
extern bool otp_update_s5k4h8s(void);

static DEFINE_SPINLOCK(imgsensor_drv_lock);
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = S5K4H8S_SENSOR_ID,
...
/*************************************************************************
* FUNCTION
*    S5K4H8S_OTP_combination_read
*
* DESCRIPTION
*    S5K4H8S otp combination( lens id is different)
*
**************************************************************************/

static UINT32 S5K4H8S_OTP_combination_read(void)
{
	...
	//write_cmos_sensor(0x0A02, 0x0F);   //Select the page to write by writing to 0xD0000A02 0x01~0x0C
	//write_cmos_sensor(0x0A00, 0x01);   //Enter read mode by writing 01h to 0xD0000A00
	//msleep(20);
	write_cmos_sensor(0x6028, 0x4000);
	write_cmos_sensor(0x602a, 0x0100);
	write_cmos_sensor(0x6f12, 0x0100);//stream off
	msleep(10);

	write_cmos_sensor(0x602a, 0x0a02);
	write_cmos_sensor(0x6f12, 0x0f00);//set page
	write_cmos_sensor(0x602a, 0x0a00);
	write_cmos_sensor(0x6f12, 0x0100);
	
	lens_id_1 = read_cmos_sensor_8(0x0A06);
	lens_id_2 = read_cmos_sensor_8(0x0A1C);
	
	printk(" ZLX--chenyunchuan=== lens_id_1: [0x0A06]=0x%x;  lens_id_2: [0x0A1C] =  0x%x  \n", lens_id_1, lens_id_2);
	if (0x2 == lens_id_1||0x2 == lens_id_2)
	{
	printk(" chenyunchuan===s5k4h8s is new lens! \n");
	return TRUE;
	}
	else
	{
	printk(" chenyunchuan===s5k4h8s err! \n");
	return FALSE;
	}	
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
			...
			*sensor_id = read_cmos_sensor(0x6F12);
			...
			(*sensor_id) += 1;//for fake id
			LOG_INF("ZLX-chenyunchuanget_s5k4h8s_id(): %d\n ", *sensor_id);
            if (*sensor_id == imgsensor_info.sensor_id) {               
                LOG_INF("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);      
               // return ERROR_NONE;
               lens_otp=S5K4H8S_OTP_combination_read();	
		if(lens_otp)
		{
			 LOG_INF("ZLX-chenyunchuan===s5k4h8s--OK--i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
			LOG_INF("ZLX-chenyunchuan===s5k4h8s-sensor and lens  id both matched!!!\n ");
			return ERROR_NONE;
		}
		else
		{
			*sensor_id = 0xFFFFFFFF;
			 LOG_INF("ZLX-chenyunchuan===-s5k4h8s sensor id matched, but lens_id is err! \n");
...
UINT32 S5K4H8S_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
