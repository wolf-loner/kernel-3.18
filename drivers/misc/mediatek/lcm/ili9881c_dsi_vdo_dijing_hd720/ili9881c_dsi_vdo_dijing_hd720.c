...
{
    #ifdef BUILD_LK
    printf("lcm_init:dijing ili9881c \n");
    #else
...
#ifdef BUILD_LK
	dprintf(0,"%s,zhangjian lk ili9881c_dsi_vdo_dijing_hd720: level = %d\n", __func__, level);
#else
	printk("%s, zhangjian kernel ili9881c_dsi_vdo_dijing_hd720 backlight: level = %d\n", __func__, level);
#endif
        ...
        printk("%s,  ili9881c_dsi_vdo_dijing_hd720 backlight: new level = 0x%x evel_high, level_low 0x%x,0x%x\n", __func__, level,level_high, level_low);
...
LCM_DRIVER ili9881c_dsi_vdo_dijing_hd720_lcm_drv = 
{
	.name			= "ili9881c_dsi_vdo_dijing_hd720",
