...
static ssize_t show_front_flashlight(struct device *dev,struct device_attribute *attr,
					char *buf)
{
  return sprintf(buf, "%u\n", gpio1_status);
}
static ssize_t store_front_flashlight(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    char *pvalue = NULL;
    int val = 0;
     if(buf != NULL && size != 0)
    {
        val = simple_strtoul(buf,&pvalue,2);
		if(val==0)
			{
		gpio_set_value(GPIO_CAMERA_FLASH_EXT2_PIN,0);
	  gpio1_status=gpio_get_value(GPIO_CAMERA_FLASH_EXT2_PIN);
...
static DEVICE_ATTR(front_flashlight, 0664, show_front_flashlight, store_front_flashlight);


static int my_val = 0;
int strobe_single_off(void);
int strobe_single_on(void);
static ssize_t show_strobe_single_on(struct device *dev,struct device_attribute *attr,
					char *buf)
{
  return sprintf(buf, "%u\n", my_val);
}
static ssize_t store_strobe_single_on(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    char *pvalue = NULL;
     if(buf != NULL && size != 0)
    {
        my_val = simple_strtoul(buf,&pvalue,2);
		if(my_val==0)
			{
		strobe_single_off();
			}
...
static DEVICE_ATTR(strobe_single_on, 0664, show_strobe_single_on, store_strobe_single_on);
...
static int mt65xx_leds_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
		...
		ret = device_create_file(&(pdev->dev), &dev_attr_front_flashlight);
              ret = device_create_file(&(pdev->dev), &dev_attr_strobe_single_on);
