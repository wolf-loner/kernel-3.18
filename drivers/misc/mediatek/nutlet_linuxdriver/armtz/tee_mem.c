...
//#define _DUMP_INFO_ALLOCATOR 1		//wliang for debug
				 ...
				 size_t size, size_t alignment)
{
	...
	if (betterchunk != NULL) {
		...
		if (betterchunk->paddr + betterchunk->size > endAddr) {
			...
			//dev_dbg(dev,"create n_chunkH=0x%p paddr=0x%p (s=%zu)\n",(void *)next_chunk,	(void *)next_chunk->paddr, next_chunk->size);
		...
		//dev_dbg(dev,"< chunkH=0x%p paddr=%p (s=%zu) align=0x%zx\n",(void *)betterchunk,(void *)betterchunk->paddr,betterchunk->size, alignment);
		      ...
		      unsigned long paddr, size_t *size)
{
	...
	list_for_each_entry(chunk, &pool->mchunks, node) {
		if (chunk->paddr == paddr) {
			...
			if (chunk->counter == 0) {
				//dev_warn(dev,"< tee_shm_pool_free() WARNING, paddr=0x%p already released\n",(void *)paddr);
				return -EINVAL;
				...
				if (chunk->node.prev != &pool->mchunks) {
					...
					if (prev->counter == 0) {
						//dev_dbg(dev,"chunkH=0x%p paddr=0x%p free ok\n",(void *)chunk,(void *)paddr);
				...
				mutex_unlock(&pool->lock);
				//dev_dbg(dev,"< paddr=0x%p  (--) refcounter is decremented ret=1\n",(void *)paddr);
				return 1;
			}
		}
	}
...
out_failed:
	...
	tee_shm_pool_dump(dev, pool, false);
#endif
	//dev_err(dev,"< tee_shm_pool_free() FAILED, pAddr=0x%p not found\n",(void *)paddr);
	return -EINVAL;
}
