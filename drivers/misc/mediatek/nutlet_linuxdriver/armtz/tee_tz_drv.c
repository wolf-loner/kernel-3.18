...
//#define  ioremap_cached ioremap_wc			//wliang
...
#endif
#ifdef CFG_TEE_OPENSSL

typedef struct {
    uint32_t bits;
    uint32_t plen;
    uint32_t qlen;
    unsigned char pq[512];
} TEE_Rsa_pq;

static void handle_rpc_func_cmd_get_pq(struct tee_tz *ptee,
						struct teesmc32_arg *arg32)
{
    ...
    get_pg = tee_shm_pool_p2v(DEV, ptee->shm_pool, params->u.memref.buf_ptr);
    Rsa_pg = (TEE_Rsa_pq *)get_pg;
    printk("%s: yj-----------------------0x%x\n", __func__,Rsa_pg->bits);
    res = get_rsa_prime(Rsa_pg->bits,Rsa_pg->pq,&(Rsa_pg->plen),Rsa_pg->pq+256,&(Rsa_pg->qlen));
    printk("%s: yj-----------------------res %d\n", __func__,res);
    if(res == 0)
...
// zhuhy,
static void handle_rpc_func_cmd_log(struct tee_tz *ptee, struct teesmc32_arg *arg32)
{
	struct teesmc32_param *params;
	char *logfb;
	u32 len;
	...
	params = TEESMC32_GET_PARAMS(arg32);
	logfb = tee_shm_pool_p2v(DEV, ptee->shm_pool, params->u.memref.buf_ptr);
	len = params->u.memref.size;
	tee_log_add(logfb, len);
...
static void handle_rpc_func_cmd(struct tee_tz *ptee, u32 parg32)
{
	...
	case NLTEE_TUI_RPC_SD_GET_ATTR:
		// not implemented yet, zhuhy ,20150415
		arg32->ret = 0;
		break;
#ifdef CFG_TEE_OPENSSL

    case NLTEE_RSA_RPC_GET_PQ:
     
		// not implemented yet, yangjun ,20160415
        printk("%s: NLTEE_RSA_RPC_GET_PQ not implemented yet ,yj\n", __func__);
        handle_rpc_func_cmd_get_pq(ptee, arg32);
		//arg32->ret = 0;
        break;
	//break;
#endif

	case NLTEE_EVENT_RPC_WAIT_FOR_EVENT:
	{
		printk("%s: NLTEE_EVENT_RPC_WAIT_FOR_EVENT not implemented yet ,zhuhy\n", __func__);
		msleep(3000);
		arg32->ret = 0;

		break;
	}
	case NLTEE_LOG_RPC_PRINT:
	{
		handle_rpc_func_cmd_log(ptee, arg32);
		arg32->ret = 0;
		break;
	}
#endif	
...
//		printk("mt_irq_get_aiar %x, \n", mt_irq_get_aiar());
//		mt_irq_dump_status(mt_irq_get_aiar());
//		mt_gic_eoi_irq(mt_irq_get_aiar());
		break;
			...
			uintptr_t parg32, struct teesmc32_arg *arg32)
{
	...
	while (true) {
		param.a0 = funcid;
		printk("befor smc a0:0x%x", (u32)param.a0);
		tee_smc_call(&param);
		ret = param.a0;
		printk("after smc a0:0x%x", (u32)ret);
...
//    e_lock_teez(ptee);
     ...
     {
         arg32->ret = TEEC_ERROR_BUSY;   //20160113
         arg32->ret_origin = TEEC_ORIGIN_COMMS;
         return ;
...
static int tz_open(struct tee_session *sess, struct tee_cmd *cmd)
{
	...
	dev_dbg(tee->dev, "> tz_open cmd->uuid:%p\n", cmd->uuid);
	if (cmd->uuid) {
		uuid = cmd->uuid->kaddr;
		dev_dbg(tee->dev, "> tz_open cmd->uuid->kaddr:%p\n", cmd->uuid->kaddr);
		dev_dbg(tee->dev, "> tz_open  uuid arg:%x %x %x\n", uuid->timeLow, uuid->timeMid, uuid->timeHiAndVersion);
	}else{
		dev_dbg(tee->dev, "> tz_open cmd->uuid is null\n");
		dev_dbg(tee->dev, "> tz_open cmd->uuid is null\n");
	}
	if (cmd->uuid)
		uuid = cmd->uuid->kaddr;
...
static int configure_shm(struct tee_tz *ptee)
{
	...
	if (ptee->shm_cached) {
		printk( "ptee->shm_cached  true\n");
		printk( "ptee->shm_cached  true\n");
		//ptee->shm_vaddr = ioremap_cache(ptee->shm_paddr, shm_size);
		ptee->shm_vaddr = ioremap_wc(ptee->shm_paddr, shm_size);	//wliang cache to cached
	
	} else
	{
		ptee->shm_vaddr = ioremap_nocache(ptee->shm_paddr, shm_size);
		printk( "ptee->shm_cached  not true\n");
		printk( "ptee->shm_cached  not true\n");
	}
...
static int tz_tee_probe(struct platform_device *pdev)
{
	...
	if (ret)
		goto bail0;
    #if 0
	ret = tee_log_init(ptee);
	if (ret != 0) {
		pr_err("failed to init tee log (rc=%d)\n",
			   ret);
		goto bail1;
	}
	#endif
