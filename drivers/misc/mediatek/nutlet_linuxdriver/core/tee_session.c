			       ...
			       struct tee_cmd_io *cmd_io)
{
	...
	if (cmd.uuid) {
		...
		dev_dbg(_DEV(tee), "%s: cmd.uuid is null\n", __func__);		//wliang
	
	dev_dbg(_DEV(tee), "%s:tee->ops->open begin UUID=%s\n", __func__,
			_uuid_to_str((TEEC_UUID *) cmd.uuid->kaddr));	//wliang
			
	ret = tee->ops->open(sess, &cmd);
	dev_dbg(_DEV(tee), "%s: tee->ops->open end, ret:%d\n", __func__, ret);		//wliang
	if (ret == 0)
...
static int tee_do_invoke_command_32(struct tee_session *sess,
				 struct tee_cmd_io_32 __user *u_cmd)
{
	...
	if (tee_copy_from_user(ctx, &k_cmd_32, (void *)u_cmd, sizeof(struct tee_cmd_io_32))) {
...
static int tee_do_cancel_cmd_32(struct tee_session *sess,
			     struct tee_cmd_io_32 __user *u_cmd)
{
	int ret = -EINVAL;
	struct tee *tee;
			      ...
			      unsigned long arg)
{
	...
	dev_dbg(_DEV(tee), "%s: > cmd nr=%d\n", __func__, _IOC_NR(cmd));
//	printk("zzzzzzzzzzzzzzzzzzzhuhy, it's ioctl32, cmd = %x TEE_INVOKE_COMMAND_IOC_32 = %x\n", cmd, (unsigned int)TEE_INVOKE_COMMAND_IOC_32);

	switch (cmd) {
	case TEE_INVOKE_COMMAND_IOC_32:
		ret =
		    tee_do_invoke_command_32(sess,
					  (struct tee_cmd_io_32 __user *)arg);
		break;
	case TEE_REQUEST_CANCELLATION_IOC_32:
		ret = tee_do_cancel_cmd_32(sess, (struct tee_cmd_io_32 __user *)arg);
		break;
	default:
		ret = -ENOSYS;
		break;
...
#ifdef CONFIG_COMPAT		// zhuhy, for 32bit process
	if (tee_is_ioctl32(ctx))
	{
		if (tee_context_copy_from_client
			(sess->ctx, &op_32, cmd_io->op, sizeof(TEEC_Operation_32)))
			goto out;
		if (tee_op_32_to_64(&op, &op_32))			
			goto out;
	}
	else {
#endif		
	...
	for (idx = 0; idx < TEEC_CONFIG_PAYLOAD_REF_COUNT; ++idx) {
		...
		case TEEC_MEMREF_WHOLE:
			if (sess->ctx->usr_client) {
				...
				if (tee_is_ioctl32(ctx))
				{
					TEEC_SharedMemory_32 shm32;
					if (tee_copy_from_user
					    (ctx, &shm32,
					     op.params[idx].memref.parent,
					     sizeof(TEEC_SharedMemory_32))) {
						res = TEEC_ERROR_BAD_PARAMETERS;
						goto out;
					}

					tee_op_shm_32_to_64(&param->c_shm[idx], &shm32);
				}
				else {
#endif					
	...
	for (idx = 0; idx < TEEC_CONFIG_PAYLOAD_REF_COUNT; ++idx) {
		...
		case TEEC_VALUE_INOUT:{
			dev_dbg(_DEV_TEE, "%s: a=%08x, b=%08x\n",
				...
				if (tee_is_ioctl32(ctx))
				{
					TEEC_Value_32 value_32;
					value_32.a = cmd->param.params[idx].value.a;
					value_32.b = cmd->param.params[idx].value.b;

					if (tee_copy_to_user
					    (ctx, &((TEEC_Operation_32 __user *)(cmd_io->op))->params[idx].value,
					     &value_32,
		...
		case TEEC_MEMREF_TEMP_INOUT:{
			...
			if (size !=
				...
				if (tee_is_ioctl32(ctx))
				{
					...
					uint32_t __user * psize = &((TEEC_Operation_32 __user *)(cmd_io->op))->params[idx].
						     tmpref.size;
//					pr_err("%s:%d - zhuhy\n", __func__, __LINE__);
					tee_put_user(ctx, size_32, psize);
				}
				else {
#endif		
				...
				if(tee_is_ioctl32(ctx))
				{
					if (size > ((TEEC_Operation_32 *)(cmd_io->op))->params[idx].tmpref.size)
						dev_err(_DEV_TEE,
						 ...
						 (ctx,  
						(void*)(uint64_t)((TEEC_Operation_32 *)(cmd_io->op))->params[idx].tmpref.buffer,
						cmd->param.params[idx].shm->kaddr,
						size))
						dev_err(_DEV_TEE,
							"%s:%d: can't update %d result to user\n",
