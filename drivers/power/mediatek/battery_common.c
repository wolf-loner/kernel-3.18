...
extern void charging_disable_by_ce_high(int ce_num);
extern void charging_enable_by_ce_low(int ce_num);
				...
				enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		if(val->intval)
		{
			charging_enable_by_ce_low(100);
			data->chg_enabled = val->intval;
			battery_log(BAT_LOG_CRTI, "enable charging");
		}
		else
		{		
			charging_disable_by_ce_high(100);
			data->chg_enabled = val->intval;
			battery_log(BAT_LOG_CRTI, "disable charging");
		}
		break;
	default :
		ret = -EINVAL;
...
static kal_bool mt_battery_100Percent_tracking_check(void)
{
	...
	if (BMT_status.bat_full == KAL_TRUE) {	/* charging full first, UI tracking to 100% */
		if (BMT_status.UI_SOC >= 100) {
			...
			//	return resetBatteryMeter; //¡¤¨¤?13?¦Ì?D???¡À¡§?¨²¡ê???UI_SOC2??¨²¡ê?¦Ì?¨¢?3???¨¬?¡À?
			}
...
static void battery_update(struct battery_data *bat_data)
{
	...
	if ((BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR)&&(bat_data->chg_enabled == 1)) {
