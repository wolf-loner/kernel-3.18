...
void alarm_set_power_on(struct timespec new_pwron_time, bool logo)
{
	...
	pr_err("alarm_set_power_on  new_pwron_time.tv_sec=%ld ",new_pwron_time.tv_sec);
	new_pwron_time.tv_sec = new_pwron_time.tv_sec-120;
	pr_err("alarm_set_power_on set 120s before new_pwron_time.tv_sec=%ld ",new_pwron_time.tv_sec);
							...
							struct timespec *ts)
{
	...
	if (alarm_type == ANDROID_ALARM_POWER_ON ||
		...
		alarm_type == ANDROID_ALARM_RTC_POWER_ON) {
		ts->tv_sec = 0;
							...
							struct timespec *ts)
{
	...
	if (alarm_type == ANDROID_ALARM_RTC_POWER_ON) {
		alarm_set_power_on(*ts, true);
		return;
	}
