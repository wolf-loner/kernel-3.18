...
#endif
		//zhenghaifeng adds for killing cameraserver when cameraserver's memory is greater than 200M
		if (!strcmp(p->comm, "cameraserver")) {
			if(tasksize * (long)(PAGE_SIZE) > 300*1024*1024){				
				send_sig(SIGKILL, p, 0);
				lowmem_print(1, "hai:lowmemory killer:%s is killed\n", p->comm);
				task_unlock(p);
				continue;
			}
		}
		//zhenghaifeng adds end	
